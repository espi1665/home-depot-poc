...
async function syncAllPrices() {
  ...
  const home_depot_items = [
    ...
    { id: "100318521", google_id: 4, name: "50 lb. Fast-Setting Concrete Mix" },
    {
      id: "205220341",
      google_id: 5,
      name: "4x4x8 #2 Ground Contact Pressure-Treated Pine Timber",
    },
    {
      id: "206970948",
      google_id: 6,
      name: "2x4x8 #2 Ground Contact Pressure-Treated Lumber",
    },
  ];
  const raw_body = {
  ...
  for (let i = 0; i < home_depot_items.length; i++) {
    console.log(
      `${"-".repeat(80)}\nName=${data.table.rows[i].c[1].v} === ${
        home_depot_items[i].name
      } \nPrice=${data.table.rows[i].c[2].v}===${
        home_depot_items[i].price
    ...
    if (
      home_depot_items[i].price > data.table.rows[i].c[4].v * 1.2 ||
