...
const home_depot_items = [
    {"id": "205757688", "name":"Western Red Cedar Dog-Ear Fence Picket"},
    {"id": "202319053", "name":"Pressure-Treated Pine Dog-Ear Fence Picket"},
    {"id": "100318511", "name":"80 lb. Concrete Mix"},
    {"id": "100318521", "name":"50 lb. Fast-Setting Concrete Mix"},
    {"id": "205220341", "name":"4 in. x 4 in. x 8 ft. #2 Ground Contact Pressure-Treated Southern Yellow Pine Timber"},
    {"id": "206970948", "name":"2 in. x 4 in. x 8 ft. #2 Ground Contact Pressure-Treated Lumber"}
    ]
...
async function syncAllPrices() {
  for (let i = 0; i < home_depot_items.length; i++) {
    ...
    try {
      ...
      console.log(`${"-".repeat(40)}\n${home_depot_items[i].name}\nPrice=${result.data.product.pricing.value}`);
    } catch (error) {
      console.error(error);
    }
  }
